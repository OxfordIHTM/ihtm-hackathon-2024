...
- `data/` contains the datasets required for this project. These
  ...
  programmaticaly by the hackathon participants and saved into this
  directly;
...
- `reports/` contains Rmarkdown file/s for producing report/s produced
  ...
  - `sudan_health_nutrition.Rmd` - Rmd file that produces the overall
    report
...
3.  Install all declared R package dependencies found in `packages.R`.
    ...
    Please make sure to replace the placeholder text
    `name_of_project1`etc with the actual names of the packages listed
    in the `packages.R` file.
